...
import (
	...
	nv21pure "github.com/filecoin-project/go-state-types/builtin/v12/migration/pure"
...
func DefaultUpgradeSchedule() stmgr.UpgradeSchedule {
	...
	updates := []stmgr.Upgrade{{
		...
		Network:   network.Version21,
		Migration: UpgradeActorsV12_5,
		PreMigrations: []stmgr.PreMigration{{
			PreMigration:    PreUpgradeActorsV12_5,
			StartWithin:     120,
			DontStartWithin: 15,
			StopWithin:      10,
		}},
...
func PreUpgradeActorsV12_5(ctx context.Context, sm *stmgr.StateManager, cache stmgr.MigrationCache, root cid.Cid, epoch abi.ChainEpoch, ts *types.TipSet) error {
	// Use half the CPUs for pre-migration, but leave at least 3.
	workerCount := MigrationMaxWorkerCount
	if workerCount <= 4 {
		workerCount = 1
	...
	_, err = upgradeActorsV12_5Common(ctx, sm, cache, lbRoot, epoch, lbts, config)
	return err
}

func UpgradeActorsV12_5(ctx context.Context, sm *stmgr.StateManager, cache stmgr.MigrationCache, cb stmgr.ExecMonitor,
	root cid.Cid, epoch abi.ChainEpoch, ts *types.TipSet) (cid.Cid, error) {
	// Use all the CPUs except 2.
	workerCount := MigrationMaxWorkerCount - 3
	if workerCount <= 0 {
	...
	newRoot, err := upgradeActorsV12_5Common(ctx, sm, cache, root, epoch, ts, config)
	if err != nil {
...
func upgradeActorsV12_5Common(
	...
	// Perform the migration
	newHamtRoot, err := nv21pure.MigrateStateTree(ctx, adtStore, manifest, stateRoot.Actors, epoch, config,
		migrationLogger{}, cache)
	if err != nil {
		return cid.Undef, xerrors.Errorf("upgrading to actors v12: %w", err)
	}
